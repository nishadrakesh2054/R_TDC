...
export const manualRegistration = {
	...
	options: {
		...
		properties: {
			noOfParticipants: {
				isVisible: ({ record }) =>
					record && record.gameType === "Individual",
			},
			totalAmount: {
				isVisible: true,
				isDisabled: true,
				...
				before: async (request) => {
					if (request.payload.gameType === "Individual") {
						request.payload.totalAmount =
							request.payload.gameFee *
							(request.payload.noOfParticipants || 1);
					} else {
				...
				handler: async (request, response, context) => {
					...
					try {
						...
						if (!fs.existsSync(excelFilePath)) {
							throw new Error(
								`Excel file not found at path: ${excelFilePath}`
							);
						}

						const { outputDir, generatedFiles } =
							await generateCertificates(excelFilePath);
						...
						return {
							...
							overlayUrls: generatedFiles.map(
								(file) =>
									`/generated_certificates/${path.basename(
										file
									)}`
							),
						};
